        ...
        //      $("#closetan").click();
              ...
              //表格为空时
               ...
               for ( var i = 1; i <$("#table_stock_before tr").length; i++) {
                ...
                var arr_xulieall=[];
               for ( var i = 1; i <$("#table_stock_before tr").length; i++) {
                       var tdid_xulie= $("#table_stock_before tr").eq(i).children().eq(11).text();
                                // 把id储存在arr数组中
								arr_xulieall.push(tdid_xulie );
			}
            //  }                 
               ...
               for ( var i = 1; i <$("#table_stock_before tr").length; i++) {
                  //     supopt.options[supindex].value
                       ...
                       var table_xu= $("#table_stock_before tr").eq(i).children().eq(11).text();
                           ...
                           last_td++;     
                         alert("添加数组通过单个供货商"+arr2);
                         alert("添加数组table序列"+arrxu);
                        
                       if(arr2.indexOf($("#add_spId").val( ))>-1){
                             var sup_spid=arr2.indexOf($("#add_spId").val( ));
                             alert("商品编号"+sup_spid);
                            var xu_id=  arrxu[sup_spid];
                           alert("table序列"+xu_id);
                       }
			     ...
			     //得到表格中已有要添加商品的id
					//  var muns=arr.indexOf($("#add_spId").val( ));
                   ...
                   //判断  当表格中已有要添加的商品时    重复添加操作                               		  //判断供货商是否相同
		            if (arr.indexOf($("#add_spId").val( )) > -1 && supopt.options[supindex].value==issup) {
                           if(jin_gs==$("#ingm_sel").val()){
					     //更改原来添加商品的数量  重复添加
					      if(confirm("该进货商已采购此商品，是否增加数量")){ 
					       $("#closetan").click();//////////////////////////////////////////
                         $("#table_stock_before tr").eq(muns+1).children().eq(6).html(parseInt($("#table_stock_before tr").eq(muns+1).children().eq(6).text())+parseInt($("#add_BuyNumber").val()));
                           }
                           }else{
			                          //   if(confirm("已有此操作,是否由   "+jin_gsname+"  更改为   "+ingmopt.options[ingmindex].text+"  并增加数量")){ 
			                         ...
			                         $("#table_stock_before tr").eq(muns+1).children().eq(4).html(wareopt.options[wareindex].value);//更改商品录入仓库id
			                         $("#table_stock_before tr").eq(muns+1).children().eq(4).html(wareopt.options[wareindex].text);//更改商品录入仓库name
 ...
 //商品添加    添加到进货单 提交审核     table_stock_before
   function stock_add(){  
    ...
    $(function (){
          ...
          url:"stock/getoffice",
          type:"post",
          data:{"name":$("#relaname").val()},
          dataType:"json",
          success:function(data){
             sel_ocId=data[0].ocId;
            }
            })
    })    
    //   查询所有商品单位
