		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer" id="vue">
					...
					<button type="button" class="btn btn-primary"   id="yes_stock">通过
					</button>
					<button type="button" class="btn btn-danger"   id="no_stock">驳回 
					</button>
 ...
 <div style="background-color: #fff;padding: 20px;">
	...
	<div id="term">
         <select id="sel_state" style="height:40px;width:150px;position:relative;top:7px;">
            ...
            <option value="1" selected="selected">一审通过</option>
...
function cli(obj){
	...
	var sel_state=$(obj).parent().parent().prev().prev().prev().prev().prev().prev().prev().children().html();
	if(sel_state!="一审通过"){//不是未审核状态是，隐藏通过驳回按钮
	       $("#yes_stock").hide();
	       $("#no_stock").hide();
	       $("#check_reason").hide();
	  }
	...
	var tableins=table.render({
	   ...
	   $("#yes_stock").click(function(){
	       if($("#check_reason").val()==null || $("#check_reason").val()=="" ){
	           if(confirm("未填写备注，是否通过审核并递交财务？")){
	                 yess();
	           }
	       }else{
	          if(confirm("是否通过审核并递交财务？")){
	                 yess();
	           }
	       }   
          })
      function yess(){
          tableins.reload({
		url: 'stock/yes_stock2',
		where:{
			 stkId:check_id,
			 checkDate2Reason:$("#check_reason").val(),
		},done:function(){
		$("#close").click();  
